...
1. **Explain what things do, not how they're built.** "Know what you own and where it is" — not
...
A household inventory — photograph your stuff, let the AI name it, and always know where it is.
```
...
- A **backend server** that stores your items, where they live, and the reminders attached to them
- **AI scanning** — photograph a shelf, get every item on it named, categorised and counted
- An **iOS app** to browse, search and move your items, and to get notified when something runs low
```
...
`.env.example` to `.env`, copy the Firebase `GoogleService-Info.plist` into `ios/Bazar/`, set up
...
**What it does:** powers photo scanning — identifying the items in a picture (Gemini 2.5 Flash).
...
Bazar's keys: `NITRO_GOOGLE_API_KEY` (required — the AI scan) and `NITRO_ADMIN_TOKEN` (required —
gates `POST /admin/migrate`). The GraphQL API authenticates via the **Firebase ID token**, so the
app needs no static API token to run.
...
- In production, secrets live in GCP Secret Manager, provisioned by `infra/secrets.tf` — never
  commit them.
...
1. Open the Xcode project: `open ios/Bazar.xcodeproj`
...
| [Business rules](docs/business-rules.md) | What the product actually models |
