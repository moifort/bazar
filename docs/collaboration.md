...
That includes the post-task code review: read your own diff, state what you checked and what you
found, in the reply.
...
`/Users/thibaut/Code/shuhari` (sibling repo) shares Bazar's exact stack and conventions (Bun +
Nitro + Pothos + Firestore repositories, SwiftUI + Apollo codegen, same DDD layout) and is where
these guides come from. When a pattern exists there, **replicate it rather than inventing one**:

- Cursor pagination in a repository (`where` + `orderBy` + `startAfter(docSnapshot)` +
  `limit(n+1)` → `hasMore`), with a hand-rolled page type `{ items, hasMore, totalCount }` — no
  Relay connections.
- Firestore composite indexes declared in `firestore.indexes.json` and provisioned by
  `infra/firestore.tf` (`jsondecode` fan-out); `terraform apply` runs in CI.
- The in-memory Firestore fake with read/write accounting (`server/test/fake-firestore.ts`) and
  the read-budget assertions it enables.
- Firestore `orderBy` silently drops docs missing the sorted field — only sort or paginate on
  fields required on every document.

`/Users/thibaut/Code/vinarium` shares the same lineage and is a second source for these patterns.
