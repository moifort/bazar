...
export namespace EntitlementQuery {
  // What the App Store sold this user, if anything. Absent is the normal state:
  // most users never subscribe.
  ...
  // notifications, which name a user only through the token they carry.
  export const byToken = async (token: AppAccountToken): Promise<Entitlement | undefined> =>
    repository.findByAppAccountToken(token)

  // The single answer to "what is this user entitled to". Every scan gate goes
  // through here.
  //
  ...
  // transaction comes back naming its user. Derived here and never on the client:
