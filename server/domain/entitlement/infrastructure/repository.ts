...
// Keyed by user: one entitlement each, a renewal overwriting it in place.
// Memoized because the plan is read on every AI call and again by the `quota`
// query — one read per request, not three.
...
// The webhook's way in: Apple's notification names the user only through the
...
// Keyed by user, so one delete by key — no query. Nothing else references the
