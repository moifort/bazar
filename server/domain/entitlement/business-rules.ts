...
const TOKEN_NAMESPACE = 'b47e5d21-9c38-4f6a-8e1d-3a5c7f0b2e94'
...
// variant bits forced) of the user's id — derived rather than stored, so it needs
// no write and is the same before and after a reinstall. One-way on purpose: the
// token travels through Apple's servers and reveals nothing about the account.
...
// Whether the App Store still owes this user their Premium. A refund revokes it
// on the spot; otherwise it runs to the paid-for date, cancelling only stops the
// renewal. Cancelled today still means Premium until the end of the period —
...
// The plan a user is on. Absent entitlement, expired entitlement, revoked
