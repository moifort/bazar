...
describe('EntitlementUseCase.sync', () => {
  test('records the purchase and turns the user Premium', async () => {
    const result = await EntitlementUseCase.sync(userId, 'eyJ...')
    if (typeof result === 'string') throw new Error('expected an entitlement')

    expect(result.productId as string).toBe('co.polyforms.bazar.premium.yearly')
  ...
  test('a renewal overwrites in place — one entitlement per user', async () => {
  ...
  test('an expired entitlement drops the user back to free', async () => {
...
describe('EntitlementUseCase.applyNotification', () => {
  test('extends a known user’s entitlement on a renewal', async () => {
    await EntitlementUseCase.sync(userId, 'eyJ...')
    const later = new Date(NEXT_YEAR.getTime() + 365 * 24 * 3600 * 1000)
  ...
  test('ignores a notification for a user we have never recorded', async () => {
