...
export namespace EntitlementCommand {
  ...
  // already been checked. Overwrites in place: a renewal is the same
  // entitlement with a later date, not a second one.
  ...
  // Everything this domain holds on one user, erased. The subscription itself is
  // Apple's to end: deleting the entitlement stops the app believing in it, it does
  // not stop the billing, which only the user can from the App Store settings.
