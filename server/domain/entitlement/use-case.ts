...
// A transaction we can act on: signed by Apple, naming a user, and granting time
// (a one-off purchase would carry no expiry — there is none in this app, but the
// wire cannot promise that).
...
export namespace EntitlementUseCase {
  ...
  // names *this* user, and only then records the Premium.
  //
  // The account-token check is what stops a signed transaction from being
  // replayed onto someone else's account: the token is derived from the user's
  ...
  //
  // A notification for a user we have never recorded is acknowledged and dropped:
  // the token is a one-way derivation, so there is nobody to attach it to until
  // the app syncs the purchase itself, which it does on the next launch.
