...
type FakeQuery = {
  ...
  get: () => Promise<{ docs: Array<{ id: string; data: () => Doc; ref: FakeRef }> }>
...
// Sentinel recorded by `update()` so a migration dropping a field can be asserted.
// Mirrors firebase-admin's FieldValue.delete() without importing the SDK.
const DELETE_SENTINEL = '__fake_delete__'
export const isDeleteSentinel = (value: unknown) =>
  value === DELETE_SENTINEL ||
  (typeof value === 'object' &&
    value !== null &&
    (value as { methodName?: string }).methodName === 'FieldValue.delete')
...
export const createFakeFirestore = () => {
  ...
  // The writes each committed transaction applied, so a test can assert that a
  // read-modify-write went through one and not through a plain read-then-set.
  ...
  // `update` merges, and drops the keys whose value is a delete sentinel — the
  // shape a null-removing migration produces.
  const applyUpdate = (collection: string, id: string, data: Doc) => {
  ...
  const makeQuery = (collection: string, state: QueryState): FakeQuery => ({
    ...
    get: async () => {
      ...
      if (state.orders.length > 0) {
        // Firestore drops documents missing the sorted field — mirror it, so a
        // test catches the silent omission instead of production doing it.
    ...
    const batch: FakeBatch = {
      ...
      commit: async () => {
        ...
        for (const op of ops) {
          ...
          else if (op.type === 'update') applyUpdate(op.ref.collection, op.ref.id, op.data)
  ...
  // Transactions run one at a time: the whole point of the primitive is that a
  // read-modify-write must not interleave with another. Real Firestore reaches it
  // by replaying a losing transaction; serializing is the same guarantee, and it
  // makes a concurrency test deterministic instead of retry-dependent.
  ...
  const runTransaction = <T>(run: (tx: FakeTransaction) => Promise<T>): Promise<T> => {
    const result = transactionQueue.then(async () => {
      ...
      for (const op of writes) {
        if (op.type === 'set') docsOf(op.ref.collection).set(op.ref.id, op.data)
        else if (op.type === 'delete') docsOf(op.ref.collection).delete(op.ref.id)
      }
