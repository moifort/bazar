#!/usr/bin/env bash
#
# bootstrap.sh — one-shot local provisioning of the Bazar Firebase stack.
#
...
# infra/.bin/ (pinned + checksum-verified).
...
EOF
  ...
  ok "State migrated to gs://${TFSTATE_BUCKET}/bazar"
...
cat <<EOF
  ...
  Terraform state      : gs://${TFSTATE_BUCKET}/bazar
  ...
  2. Add the following GitHub secrets / variables:
       Secrets : GCP_BILLING_ACCOUNT, GCP_WIF_PROVIDER, GCP_DEPLOY_SA,
                 ...
                 APPLE_PRIVATE_KEY_P8, GEMINI_API_KEY, ADMIN_TOKEN
       Vars    : GCP_PROJECT_ID (= ${PROJECT_ID}), GCP_ORG_ID, IOS_BUNDLE_ID
