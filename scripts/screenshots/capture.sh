...
#
# Usage: scripts/screenshots/capture.sh <path-to-Bazar.app> <output-dir>
set -euo pipefail

APP_PATH="${1:?usage: capture.sh <app-path> <output-dir>}"
OUT_DIR="${2:?usage: capture.sh <app-path> <output-dir>}"

BUNDLE_ID="co.polyforms.bazar"
...
# The store panels are shot in light appearance; the inventory is a light-first app.
...
for SCREEN in $SCREENS; do
  ...
  if [ -z "$captured" ]; then
    ...
    xcrun simctl spawn "$UDID" log show --last 3m --style compact \
      --predicate 'process == "Bazar"' 2>/dev/null | tail -40 >&2 || true
    exit 1
  fi
