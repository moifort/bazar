...
/// What the server says a user is entitled to. `appAccountToken` is the UUID a
...
enum SubscriptionAPI {
    static func load() async throws -> EntitlementState {
        let data = try await GraphQLHelpers.fetch(
            ...
            query: BazarGraphQL.EntitlementQuery()
        )
        return state(
    ...
    static func sync(signedTransaction: String) async throws -> EntitlementState {
        let data = try await GraphQLHelpers.perform(
            GraphQLClient.shared.apollo,
            mutation: BazarGraphQL.SyncEntitlementMutation(signedTransaction: signedTransaction)
        )
        return state(
    ...
    private static func state(
        plan: GraphQLEnum<BazarGraphQL.Plan>,
        token: String,
        productId: String?,
        expiresOn: String?
    ) -> EntitlementState {
