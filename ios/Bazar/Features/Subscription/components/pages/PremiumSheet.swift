...
struct PremiumSheet: View {
    ...
    /// The yearly price is the one put forward, so it is pre-selected.
    ...
    private var benefits: some View {
        VStack(alignment: .leading, spacing: 16) {
            BenefitRow(
                ...
                title: "Scannez sans compter",
                detail: "Plus d'allocation mensuelle à surveiller."
            )
            BenefitRow(
                icon: "shippingbox",
                title: "Rangez une maison entière",
                detail: "Une pièce, un garage, une cave — d'une traite."
            )
            BenefitRow(
                icon: "heart",
                title: "Soutenez l'app",
                detail: "Chaque scan a un coût : l'abonnement le paie."
            )
        }
    ...
    private var subscribeBar: some View {
        VStack(spacing: 8) {
            ...
            // sheet as a user sees it, CTA included.
    ...
    private func offer(_ product: Product) -> Offer {
        ...
        let trialDetail = trial.map { offer in
            "\(offer.period.value) \(offer.period.unit.frenchName(offer.period.value)) d'essai gratuit, puis renouvellement automatique"
        }
        return Offer(
            id: product.id,
            title: product.displayName,
            price: product.displayPrice,
            detail: isTrial ? (trialDetail ?? "sans engagement") : "sans engagement",
            badge: product.id == SubscriptionProducts.yearly ? "Économisez 16 %" : nil,
            isTrial: isTrial
        )
    }
}
...
private extension Product.SubscriptionPeriod.Unit {
    ...
    /// "N <unité> d'essai gratuit" precisely to dodge the gender agreement an
