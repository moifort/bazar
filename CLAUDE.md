...
- **One item groups identical units, its `tags` say them apart**: twelve labelled jars are one item with `quantity: 12` and the labels in `tags`, alongside the words describing the object. Always present (empty, never `null`), deduplicated case- and accent-insensitively, capped at 20, and fed to the search index next to name, description and notes.
